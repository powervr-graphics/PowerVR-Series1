 ...
 * Modifications:
 * $Log: texas.c,v $
 * Revision 1.3  1997/04/02  19:41:19  mjg
 * Included the calling convention on TexasWriteBMP.
 *
 * Revision 1.2  1997/03/31  22:14:26  jop
 * Got rid of windows references
 *
 * Revision 1.1  1996/11/14  14:15:15  erf
 * Initial revision
 *
 * Revision 1.1  1996/11/14  13:00:36  erf
 * Initial revision
 *
 * Revision 1.3  1996/04/22  15:50:47  pdl
 * changed the texture interface to 32 bits.
 *
 * Revision 1.2  1996/02/19  14:20:04  sjf
 * Moved Twiddle functions to txmops
 *
 * Revision 1.1  1995/12/18  14:22:36  sjf
 * Initial revision
 *
...
#define PIXEL_CAM_SIZE 8
unsigned long PixelCam[2][PIXEL_CAM_SIZE];
unsigned long PixelCamMiss=0;
unsigned long PixelCamHit=0;
unsigned long PCXCache=0;
unsigned long PCXCacheNU=0;
unsigned long HighestCount=0;
unsigned long LowestCount=0;

unsigned long ShuffCount=0;
int	CurrentTag=-1;

unsigned long BreakCount=0;
unsigned long DummyBreakCount=0;

unsigned long TextureCallCount=0;

unsigned short cfrScale;			/*the scalar for 'c','f' and 'r' */

unsigned long textureMemory[TEXTURE_MEMORY_SIZE>>1]; /* changed to 32 bit words*/
...
FILE *FrawTexPixels;
FILE *FtexasInput;
FILE *FtexasInputSabOutputFormat; /*I'm having a laugh*/
FILE *Fpixels;
FILE *Fbilin1;
FILE *Fbilin2;
FILE *Fbilin3;
...
void InitTexasSimulator()
{
	...
	FsabreFloat=fopen("params/sabrefloat.txt","wb");
	...
	FtexasInputSabOutputFormat=fopen("params/sabreout.txt","wb"); 
	Fpixels=fopen("params/pixels.txt","wb"); 
    Fbilin1=fopen("params/bilin1.txt","wb");
    Fbilin2=fopen("params/bilin2.txt","wb");
    Fbilin3=fopen("params/bilin3.txt","wb");
    Fbilin4=fopen("params/bilin4.txt","wb");
...
void FinishTexasSimulator()
{
	...
	fclose(FtexasInput); 
	fclose(FtexasInputSabOutputFormat);
	fclose(Fpixels); 
	fclose(Fbilin1);
	fclose(Fbilin2);
	fclose(Fbilin3);
...
void DumpTextureMemory()
{
	...
	*/
	...
	for(i=0;i<TEXTURE_MEMORY_SIZE>>1;i++)
	{
		/*
		// Dump in little endian mode
		*/

		putc((textureMemory[i])    & 0xff,	dumpFile);
		putc((textureMemory[i]>>8) & 0xff,	dumpFile);
		putc((textureMemory[i]>>16)& 0xff,	dumpFile);
		putc((textureMemory[i]>>24)& 0xff,	dumpFile);
...
void FetchPixelDummy(unsigned long address)
{
...
void ShufflePixels(int NextX,int NextY,int NextTag,unsigned char mapSize,int whichMap,
				   unsigned char colourDepth,unsigned char mipMapped,unsigned char flipUV)

{
	...
	static unsigned long lastFetched;
	int TextSizeX,TextSizeY;
    int DiffX,DiffY;
	int bit8;
	int Reorder;
	long binc;	
	unsigned long AddrA,AddrB,AddrC,AddrD;
    ...
    AddrA= AddressCalc(NextX<<(whichMap+bit8),NextY<<(whichMap+1),
					   NextTag,mapSize,whichMap,colourDepth,mipMapped,flipUV)>>1;
	AddrB= AddressCalc((NextX+1)<<(whichMap+bit8),NextY<<(whichMap+1),
					   NextTag,mapSize,whichMap,colourDepth,mipMapped,flipUV)>>1;
	AddrC= AddressCalc(NextX<<(whichMap+bit8),(NextY+1)<<(whichMap+1),
					   NextTag,mapSize,whichMap,colourDepth,mipMapped,flipUV)>>1;
	AddrD= AddressCalc((NextX+1)<<(whichMap+bit8),(NextY+1)<<(whichMap+1),
					   NextTag,mapSize,whichMap,colourDepth,mipMapped,flipUV)>>1;

	NextTag|=whichMap<<24; /*to make each map unique*/


	TextSizeX=mapMask[mapSize]>>(whichMap+bit8);
	TextSizeY=mapMask[mapSize]>>(whichMap+1); /*32 bit word*/


    DiffX=(NextX-CurrentX+1) & TextSizeX;
    DiffY=(NextY-CurrentY+1) & TextSizeY;

    CurrentX=NextX;
    CurrentY=NextY;

	Reorder=((AddrA & 0xfffffe00) != (AddrB & 0xfffffe00));

	if(Reorder)
		Reorder=1;

/*	CurrentTag=-1;  just for mark*/


    if(NextTag==CurrentTag && DiffX==0 && DiffY==0)
    {
    	/*left and up case*/
		ShuffCount+=3;
   	    fprintf(Fbilin4,"%d %d %d\n",AddrA,AddrB,AddrC);
		if(Reorder)
		{
			FetchPixelDummy(AddrA);
			FetchPixelDummy(AddrC);
			FetchPixelDummy(AddrB);
		}
		else
		{
			FetchPixelDummy(AddrA);
    ...
    else if(NextTag==CurrentTag && DiffX==1 && DiffY==0)
    {
    	/*up case*/
		ShuffCount+=2;
   	    fprintf(Fbilin4,"%d %d\n",AddrA,AddrB);
    ...
    else if(NextTag==CurrentTag && DiffX==2 && DiffY==0)
    {
    	/*right and up case*/
		ShuffCount+=3;
   	    fprintf(Fbilin4,"%d %d %d\n",AddrA,AddrB,AddrD);
		FetchPixelDummy(AddrA);
		FetchPixelDummy(AddrB);
		FetchPixelDummy(AddrD);
		lastFetched=AddrD;
    }
    else if(NextTag==CurrentTag && DiffX==0 && DiffY==1)
    {
    	/*left case*/
		ShuffCount+=2;
   	    fprintf(Fbilin4,"%d %d\n",AddrA,AddrC);
    ...
    else if(NextTag==CurrentTag && DiffX==1 && DiffY==1)
    {
    	/*no movement case*/
		/* ian's sdram interface HAS to fetch something */

		ShuffCount+=1;
   	    fprintf(Fbilin4,"%d\n",lastFetched); 

    }
    else if(NextTag==CurrentTag && DiffX==2 && DiffY==1)
    {
    	/*right case*/
		ShuffCount+=2;
   	    fprintf(Fbilin4,"%d %d\n",AddrB,AddrD);
    ...
    else if(NextTag==CurrentTag && DiffX==0 && DiffY==2)
    {
    	/*left and down case*/
		ShuffCount+=3;
   	    fprintf(Fbilin4,"%d %d %d\n",AddrA,AddrC,AddrD);
    ...
    else if(NextTag==CurrentTag && DiffX==1 && DiffY==2)
    {
    	/*down case*/
		ShuffCount+=2;
   	    fprintf(Fbilin4,"%d %d\n",AddrC,AddrD);
    ...
    else if(NextTag==CurrentTag && DiffX==2 && DiffY==2)
    {
    	/*right and down case*/
		ShuffCount+=3;
   	    fprintf(Fbilin4,"%d %d %d\n",AddrB,AddrC,AddrD);
    ...
    {
    	/*all pixels must be flushed*/

    	CurrentTag=NextTag;
		ShuffCount+=4;
   	    fprintf(Fbilin4,"%d %d %d %d\n",AddrA,AddrB,AddrC,AddrD);
		if(Reorder)
		{
			FetchPixelDummy(AddrA);
			FetchPixelDummy(AddrC);
...
post	|-		 
=========================================================================*/
CAMSim(unsigned long address, unsigned long Pixel)
{
	static CAMReplace=0; /* which element to replace*/
	int i,Matched;

	i=0;
	Matched=0;
	while(i<PIXEL_CAM_SIZE && Matched==0)
	{
		if(PixelCam[0][i]==address)
		{
			Matched=1;
			ASSERT(PixelCam[1][i]==Pixel); /* to test the CAM is working*/
		}
		i++;
	}
	...
	if(Matched==0)
	{
		/* we have a miss !*/
		PixelCamMiss++;
		PixelCam[0][CAMReplace]=address;
		PixelCam[1][CAMReplace]=Pixel; /* to test the CAM is working*/
		CAMReplace=(CAMReplace+1) % PIXEL_CAM_SIZE;
...
unsigned short FetchPixel(unsigned long address)
{
	...
	if(address < (TEXTURE_MEMORY_SIZE/2))
		...
		PixelWord=textureMemory[address>>1];

	else if( (address >= BIG_BANK) && (address < (BIG_BANK+(TEXTURE_MEMORY_SIZE/2))) )

		PixelWord=textureMemory[((address ^ BIG_BANK)+(TEXTURE_MEMORY_SIZE/2))>>1];
...
/*	CAMSim(address>>1,PixelWord); */

	if(lastMiss!=PixelCamMiss)
	{

		lastMiss=PixelCamMiss;
/*   		pixel=0xfff0; */
...
void WritePixel(unsigned long address,unsigned short pixel)
{
	...
	if(address < (TEXTURE_MEMORY_SIZE/2))
	{
		...
		if(address & 1)
		{
		    PixelWord=(PixelWord & 0xFFFF0000ul) | pixel;
		}
		else
		{
			PixelWord=(PixelWord & 0xFFFFul) | ((unsigned long)pixel<<16);
		}

		textureMemory[address>>1]=PixelWord;
	...
	else if( (address >= BIG_BANK) && (address < (BIG_BANK+(TEXTURE_MEMORY_SIZE/2))) )
	{
		...
		textureMemory[((address ^ BIG_BANK)+(TEXTURE_MEMORY_SIZE/2))>>1]=PixelWord;
...
void WriteTwoPixels(unsigned long address,unsigned long pixels)
{
	...
	if(address < (TEXTURE_MEMORY_SIZE/2))
	{
		...
		textureMemory[address>>1]=pixels;
	}
  
	else if( (address >= BIG_BANK) && (address < (BIG_BANK+(TEXTURE_MEMORY_SIZE/2))) )
	{
		textureMemory[((address ^ BIG_BANK)+(TEXTURE_MEMORY_SIZE/2))>>1]=pixels;
	}
	else
...

{
	...
	RGBA tcol,high_pixel,low_pixel,u1,v1,u1v1,InAB,InCD;
	long abc,def,pqr;
	int shift;
	int powerTwo;
	pfloat top,bot;
...
/*	if(TextureCallCount==23652)
		TextureCallCount=23652; */
...
{
   	    fprintf(Fbilin1,"%d %d %d %d\n",uFrac,vFrac,address>>1,1);
		
		uFrac &= 31;
		vFrac &= 31;

   	    fprintf(Fbilin2,"%d %d %d ",uFrac,vFrac,raw_pixel);

		ShufflePixels(u,v,address,mapSize,0,colourDepth,mipMapped,flipUV);
	
		high_res_address=AddressCalc((u+1)& 255,v,address,mapSize,0,colourDepth,mipMapped,flipUV);

		raw_pixel=FetchPixel(high_res_address);

   	    fprintf(Fbilin2,"%d ",raw_pixel);

		u1=ColourConvert(raw_pixel,colourDepth,(u+1) & 1,col4444or555);
 
		/* fetch v1 */

		high_res_address=AddressCalc(u,(v+1)& 255,address,mapSize,0,colourDepth,mipMapped,flipUV);

		raw_pixel=FetchPixel(high_res_address);

   	    fprintf(Fbilin2,"%d ",raw_pixel);

		v1=ColourConvert(raw_pixel,colourDepth,u & 1,col4444or555);
 
		/* fetch u1v1 */

		high_res_address=AddressCalc((u+1)& 255,(v+1)& 255,address,mapSize,0,colourDepth,mipMapped,flipUV);

		raw_pixel=FetchPixel(high_res_address);

   	    fprintf(Fbilin2,"%d\n",raw_pixel);

		u1v1=ColourConvert(raw_pixel,colourDepth,(u+1) & 1,col4444or555);
 

		InAB.Red  =(tcol.Red  <<3) + 4 + ((u1.Red  -tcol.Red  )*uFrac>>2);
		InAB.Green=(tcol.Green<<3) + 4 + ((u1.Green-tcol.Green)*uFrac>>2);
		InAB.Blue =(tcol.Blue <<3) + 4 + ((u1.Blue -tcol.Blue )*uFrac>>2);
		InAB.Alpha=(tcol.Alpha) +        ((u1.Alpha-tcol.Alpha)*uFrac>>5);

		InCD.Red  =(v1.Red  <<3) + 4 + ((u1v1.Red  -v1.Red  )*uFrac>>2);
		InCD.Green=(v1.Green<<3) + 4 + ((u1v1.Green-v1.Green)*uFrac>>2);
		InCD.Blue =(v1.Blue <<3) + 4 + ((u1v1.Blue -v1.Blue )*uFrac>>2);
		InCD.Alpha=(v1.Alpha) +        ((u1v1.Alpha-v1.Alpha)*uFrac>>5);

   	    fprintf(Fbilin3,"%d %d %d %d %d %d %d %d\n",InAB.Red ,InAB.Green,InAB.Blue,InAB.Alpha,
												    InCD.Red ,InCD.Green,InCD.Blue,InCD.Alpha);

		red  =((int)InCD.Red   - InAB.Red  )*vFrac; 
		green=((int)InCD.Green - InAB.Green)*vFrac; 
		blue =((int)InCD.Blue  - InAB.Blue )*vFrac; 
		alpha=((int)InCD.Alpha - InAB.Alpha)*vFrac; 

		tcol.Red  =(red  >>5)+(int)InAB.Red;
		tcol.Green=(green>>5)+(int)InAB.Green;
		tcol.Blue =(blue >>5)+(int)InAB.Blue;
		tcol.Alpha=(alpha>>5)+(int)InAB.Alpha;

/*		tcol.Red=tcol.Red + (u1.Red-tcol.Red)*(uFrac/1.0) + 
							(v1.Red-tcol.Red)*(vFrac/1.0) + 
							(u1v1.Red-v1.Red-u1.Red+tcol.Red)*(uFrac/1.0)*(vFrac/1.0);
		tcol.Green=tcol.Green + (u1.Green-tcol.Green)*(uFrac/1.0) + 
								(v1.Green-tcol.Green)*(vFrac/1.0) + 
								(u1v1.Green-v1.Green-u1.Green+tcol.Green)*(uFrac/1.0)*(vFrac/1.0);
		tcol.Blue=tcol.Blue + (u1.Blue-tcol.Blue)*(uFrac/1.0) + 
							  (v1.Blue-tcol.Blue)*(vFrac/1.0) + 
							  (u1v1.Blue-v1.Blue-u1.Blue+tcol.Blue)*(uFrac/1.0)*(vFrac/1.0);
*/
}
	...
	else if(mapSize+bot.e>8)
	{
		...
		*/
		...
		tcol.Blue =(tcol.Blue <<3)+4;
   	    ...
   	    fprintf(Fbilin1,"%d %d %d %d\n",uFrac,vFrac,address>>1,9);

		CurrentTag=-1; /* so that the next shuffle fetches all 4 pixels*/
  	    fprintf(Fbilin4,"%d\n",low_res_address>>1); 
/*   	    fprintf(Fbilin4,"%d %d %d\n",x,y,high_res_address>>1); */
...
if(1)
{
		Compress=bot.e-1; 
   	    ...
   	    fprintf(Fbilin1,"%d %d %d %d\n",uFrac,vFrac,address>>1,Compress+1);
		...
		high_res_address=AddressCalc(u,v,address,mapSize,Compress,colourDepth,mipMapped,flipUV);

		ShufflePixels(u,v,address,mapSize,Compress,colourDepth,mipMapped,flipUV);

		raw_pixel=FetchPixel(high_res_address);

   	    fprintf(Fbilin2,"%d %d %d ",uFrac,vFrac,raw_pixel);

		high_pixel=ColourConvert(raw_pixel,colourDepth,u & 1,col4444or555);
 
		/* fetch u1 */
		binc=1<<(Compress);

		high_res_address=AddressCalc((u+binc)& 255,v,address,mapSize,Compress,colourDepth,mipMapped,flipUV);

		raw_pixel=FetchPixel(high_res_address);

   	    fprintf(Fbilin2,"%d ",raw_pixel);

		u1=ColourConvert(raw_pixel,colourDepth,u & 1,col4444or555);

		/* fetch v1 */

		high_res_address=AddressCalc(u,(v+binc)& 255,address,mapSize,Compress,colourDepth,mipMapped,flipUV);
		...
		high_res_address=AddressCalc((u+binc)& 255,(v+binc)& 255,address,mapSize,Compress,colourDepth,mipMapped,flipUV);

		raw_pixel=FetchPixel(high_res_address);

   	    fprintf(Fbilin2,"%d\n",raw_pixel);

		u1v1=ColourConvert(raw_pixel,colourDepth,u & 1,col4444or555);
 

		InAB.Red  =(high_pixel.Red  <<3) + 4 + ((u1.Red  -high_pixel.Red  )*uFrac>>2);
		InAB.Green=(high_pixel.Green<<3) + 4 + ((u1.Green-high_pixel.Green)*uFrac>>2);
		InAB.Blue =(high_pixel.Blue <<3) + 4 + ((u1.Blue -high_pixel.Blue )*uFrac>>2);
		InAB.Alpha=(high_pixel.Alpha) +        ((u1.Alpha-high_pixel.Alpha)*uFrac>>5);
...
/*		high_pixel.Red=high_pixel.Red + (u1.Red-high_pixel.Red)*(uFrac/1.0) + 
							(v1.Red-high_pixel.Red)*(vFrac/1.0) + 
							(u1v1.Red-v1.Red-u1.Red+high_pixel.Red)*(uFrac/1.0)*(vFrac/1.0);
		high_pixel.Green=high_pixel.Green + (u1.Green-high_pixel.Green)*(uFrac/1.0) + 
								(v1.Green-high_pixel.Green)*(vFrac/1.0) + 
								(u1v1.Green-v1.Green-u1.Green+high_pixel.Green)*(uFrac/1.0)*(vFrac/1.0);
		high_pixel.Blue=high_pixel.Blue + (u1.Blue-high_pixel.Blue)*(uFrac/1.0) + 
							  (v1.Blue-high_pixel.Blue)*(vFrac/1.0) + 
							  (u1v1.Blue-v1.Blue-u1.Blue+high_pixel.Blue)*(uFrac/1.0)*(vFrac/1.0);
...
/*   	    fprintf(Fbilin4,"%d\n",high_res_address>>1); */
   	    fprintf(Fbilin4,"%d %d %d\n",x,y,high_res_address>>1);
		if(low_res_address!=last_low_res_address)
		{
/*	   	    fprintf(Fbilin4,"%d\n",low_res_address>>1); */
	   	    fprintf(Fbilin4,"%d %d %d\n",x,y,high_res_address>>1);
		}
		/*for pcx1 cache simulation*/
		if(low_res_address==last_low_res_address)
			PCXCache++;
		else
			PCXCacheNU++;
			
		last_low_res_address=low_res_address;
...
/*	   	if(lastMiss!=missCount)
	...
	if(x==639 && y==479)
	{
		printf("ShuffCount=%d PixelCamMiss=%d PixelCamHit=%d PCXCache=%d PCXCacheNU=%d HighestCount=%d LowestCount=%d\n",
			ShuffCount,PixelCamMiss,PixelCamHit,PCXCache,PCXCacheNU,HighestCount,LowestCount);

	}
	if(x==319 && y==255)
	{
		printf("BreakCount=%d DummyBreakCount=%d\n",BreakCount,DummyBreakCount);
